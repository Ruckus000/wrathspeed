...
public enum HistoryInsights {
    public static func weeklySummary(
        ...
        let workouts = plan?.workouts.filter { $0.date >= weekStart && $0.date < end && $0.blueprint.kind.isRunning } ?? []
        let plannedMeters = workouts.reduce(0) { $0 + $1.blueprint.plannedDistanceMeters }
        let plannedRunCount = workouts.filter { $0.status == .scheduled || $0.status == .convertedToEasy || $0.status == .completed }.count
        let confirmed = workouts.filter { $0.status == .completed }.count

        let weekResults = results.filter { $0.startedAt >= weekStart && $0.startedAt < end }
        let actualMeters = weekResults.reduce(0) { $0 + $1.distanceMeters }
        let unmatchedExtra = weekResults.filter { result in
            result.matchInfo.state != .matched && result.source != .wrathspeedPhone && result.source != .wrathspeedWatch
        }.reduce(0) { $0 + $1.distanceMeters }
        let qualityCompleted = workouts.filter { $0.status == .completed && $0.blueprint.kind.isQuality }.count
